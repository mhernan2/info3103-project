...
UPDATE gifts
    SET received=1
    WHERE gift_id=gift_id_in AND to_user=user_id_in AND from_user IS NOT NULL;
    SELECT * FROM gifts WHERE gift_id=gift_id_in AND to_user=user_id_in AND from_user IS NOT NULL;
END//
DELIMITER ;
